"""LS fit P1-P18, matching production windowing, with per-period window length.
...
Usage: py fit_p18.py [db_path] [--win7|--win14]
"""
import sqlite3
import sys
...
DB = argv[0] if argv else 'fuel_prices_p18.db'
...
PARAMS = {
    'es95':         {'density': 0.755, 'premium': 0.1545, 'excise': 0.45600,
                     'src': 'RB=F', 'alts': ['GASOIL_USD', 'HO=F', 'BZ=F']},
...
def baseline(con, rates_sorted):
    ...
    for fuel, prod in PRODUCTION.items():
        ...
        new = [e for pn, e in zip(periods, errs) if pn in ('P15', 'P16', 'P17', 'P18')]
        mae = sum(abs(e) for e in errs) / len(errs)
        mae_n = sum(abs(e) for e in new) / len(new) if new else float('nan')
        print('\n%s (%s f=%s o=%s)' % (fuel, prod['src'], prod['factor'], prod['offset']))
        print('  ' + ' '.join(line))
        print('  MAE(sve)=%.1fc   MAE(P15-P18, out-of-sample)=%.1fc' % (mae * 100, mae_n * 100))
